...
public class AutoCommands {
    ...
    public static Command algaeAuto(ManipulatorSubsystem manipSub, boolean mirroredX, boolean mirroredY){
        if(mirroredY) mirroredX = !mirroredX;
        return Commands.sequence(
            Commands.parallel(
                new PathPlannerAuto("Algae 1", mirroredX),
                new GoToCmd(manipSub)// add positions later, L4 place
                // add has coral later
            ),
            // Place Coral command here,
            Commands.parallel(
                new PathPlannerAuto("Algae 1.5", mirroredX),
                new GoToCmd(manipSub)// add positions later, Algae L2 collect
            ),
            //Collect Algae L2 here,
            Commands.parallel(
                new PathPlannerAuto("Algae 2", mirroredX),
                Commands.sequence(
                    Commands.waitSeconds(0.4),
                    new GoToCmd(manipSub)
                )
